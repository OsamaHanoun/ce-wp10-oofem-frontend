...
import drawResultsFigure from "./draw-results-figure.js";
...
export default function runVisualizer(type) {
  if (type === "original") {
    const uploadedFileJSON = sessionStorage.getItem("uploadedFile");
    const data = JSON.parse(uploadedFileJSON);
    init(data);
  } else if (type === "results") {
    const downloadFileJSON = sessionStorage.getItem("downloadFile");
    const data = JSON.parse(downloadFileJSON);
    initResultsModel(data);
  }
...
function initResultsModel(data) {
  // renderer
  const container = document.getElementById("renderer-container");
  renderer = new THREE.WebGLRenderer({ antialias: true });
  renderer.setSize(window.innerWidth, window.innerHeight);
